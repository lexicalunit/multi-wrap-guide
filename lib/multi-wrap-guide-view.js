...
export default class MultiWrapGuideView {
  ...
  render () {
    return (<div class='multi-wrap-guide-view'></div>)
  }
  ...
  handleEvents () {
    ...
    this.subs.add(this.editorElement, 'mousemove', e => {
    ...
    this.subs.add(atom.commands.add('atom-workspace', {
      'multi-wrap-guide:create-vertical-guide': () => this.createVerticalGuide(this.currentCursorColumn),
      'multi-wrap-guide:create-horizontal-guide': () => this.createHorizontalGuide(this.currentCursorRow),
      'multi-wrap-guide:remove-guide': () => this.removeGuide(this.currentCursorRow, this.currentCursorColumn),
      'multi-wrap-guide:line-break': () => this.lineBreak(this.currentCursorColumn)
    }))
  ...
  removeNearPosition (pos, list) {
    ...
    for (let i of [pos, pos - 1, pos + 1, pos - 2, pos + 2]) {
  ...
  lineBreak (column) {
    if (atom.workspace.getActiveTextEditor() !== this.editor) return
    for (let i of [column, column - 1, column + 1, column - 2, column + 2]) {
  ...
  lock () {
    ...
    for (let guide of this.element.children) {
  ...
  mouseUpGuide (guide, event) {
    ...
    let positions = []
    for (let tip of this.element.querySelectorAll(query)) {
